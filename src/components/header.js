...
const Header = () => {
    ...
    const toggleProductOverlay = () => {
        setProductOverlayOpen(!productOverlayOpen);
        if (productOverlayOpen === false) {
            document.getElementById('arrowji').innerHTML = '&#11165'
        } else (
            document.getElementById('arrowji').innerHTML = '&#11167'
       )
    };
    ...
    <header>
        ...
        <nav className={`header-part nav-links ${menuOpen ? 'open' : ''}`}>
            <div className='productcontainer'>
                <a href id='producta' onClick={toggleProductOverlay}>Product  <span id='arrowji'> ⮟</span></a>
                <div
                    className={`productoverlay ${productOverlayOpen ? 'hi' : ''}`}
                    style={{ display: productOverlayOpen ? 'block' : 'none' }} 
                >
